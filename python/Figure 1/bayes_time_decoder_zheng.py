...
def decode_dataset(
    ...
    *,
    ...
    """
    Load neural data, align trials across neurons, decode time bins
    with Gaussian Naive Bayes, and return predictions and errors.
    """
    rng = np.random.default_rng(random_state)

    # ---- Load MATLAB data ----
    data = sio.loadmat(mat_filepath, squeeze_me=True)
    neural_data_struct = data["neural_data"]  # (n_neurons,)

    # ---- Extract firing rates and equalize trial counts ----
    firing_rates_list = [
        np.asarray(neuron["firing_rates"]) for neuron in neural_data_struct
    ]
    min_trials = min(fr.shape[0] for fr in firing_rates_list)
    truncated = [fr[:min_trials, :] for fr in firing_rates_list]

    # ---- Reshape to (time_bins, trials, neurons) ----
    ...
    # ---- Convert bin error to time error ----
    errors = bin_size * np.abs(y_pred - y_test)

    # ---- Compute time-bin centers ----
    if time_bin_range is not None:
        unique_bins = np.arange(start_bin, end_bin)
    else:
        unique_bins = np.arange(time_bins)
...
def shuffled_baseline(
    ...
    *,
    ...
    """
    Generate a shuffle baseline by permuting time labels.
    """
...
def add_sig_bar(ax, x1, x2, y, p, lw=2):
    ...
    ax.text((x1 + x2) / 2, y, significance_from_p(p),
...
def per_bin_ttests(
    ...
    """
    Welch t-test per time bin between two error curves.
    """
    ...
    for i, b in enumerate(bins):
        ...
        if ea.size < 2 or eb.size < 2:
            ...
            t_vals[i], p_vals[i] = ttest_ind(ea, eb, equal_var=False)
...
if __name__ == "__main__":
    ...
    TC_FILE  = "Figure 1/NBHB4s_data.mat"
    NTC_FILE = "Figure 1/NBHB4s_data_non.mat"
    ...
    # ---- Bin-wise statistics ----
    t_vals_bin, p_vals_bin = per_bin_ttests(
        err_TC, y_test_TC, err_NT, y_test_NT, unique_bins
    )
    ...
    plt.fill_between(tcent_NT, mean_err_NT - sem_err_NT, mean_err_NT + sem_err_NT,
    ...
    plt.fill_between(tcent_TC, bl_mean_TC - bl_sem_TC, bl_mean_TC + bl_sem_TC,
                     color="grey", alpha=0.15)

    margin = 0.02
    for tc, m, se, p in zip(tcent_TC, mean_err_TC, sem_err_TC, p_vals_bin):
        if np.isfinite(p) and p < 0.05:
            plt.text(tc, m + se + margin, significance_from_p(p),
                     ha="center", va="bottom", fontsize=11)
    ...
    ax.bar(group_labels, group_means, yerr=group_sems,
           color=["blue", "green", "grey"], capsize=4, alpha=0.85)
    ...
    for k, (i, j) in enumerate(pairs, 1):
        _, p_val = ttest_ind(groups[i], groups[j], equal_var=False)
        add_sig_bar(ax, i, j, ymax + 0.03 * k, p_val)

    plt.tight_layout()
    plt.show()

    print("Per-bin p-values (first 10):",
          np.array2string(p_vals_bin[:10], precision=3, separator=", "))
