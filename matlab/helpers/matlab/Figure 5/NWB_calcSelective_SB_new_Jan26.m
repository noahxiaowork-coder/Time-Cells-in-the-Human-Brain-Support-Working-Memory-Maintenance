function [sig_cells, areasSternberg, time_fields, patient_time_cell_count, ...
          brain_region_time_cell_count, time_cell_info] = NWB_calcSelective_SB_new(nwbAll, all_units, params)
...
%   time_cell_info           - [patient_id, unit_id, max_fr_bin] for time cells
    ...
    % gaussian_sigma = 2; 
    % kernel_size = round(5 * gaussian_sigma); %R you changing it?
    % x = -kernel_size:kernel_size;
    % gaussian_kernel = exp(-(x.^2) / (2 * gaussian_sigma^2));
    % gaussian_kernel = gaussian_kernel / sum(gaussian_kernel);

    gaussian_sigma_bins = 1;  % SD = 1 bin = 100 ms
    gaussian_kernel = GaussianKernal(gaussian_sigma_bins, 2);  % ±2 bins → 5 bins total
    ...
    all_units = applyRateFilter(all_units, rateFilter);
...
function p_value = time_cell_analysis_time_level(fr_per_bin_all_tc, spike_times, ...
                                                 ...
                                                 num_permutations, window_size)
%TIME_CELL_ANALYSIS_TIME_LEVEL Permutation-based time cell analysis.
%   fr_per_bin_all_tc is the observed average (already smoothed+zscored).
%   We shuffle spikes across bins, re-smooth+zscore, and compare.
    ...
    % Should match main pipeline:
    gaussian_sigma_bins = 1; 
    gaussian_kernel = GaussianKernal(gaussian_sigma_bins, 2);
