...
ndx = find([neural_data.patient_id]==subject_id & [neural_data.unit_id]==unit_id, 1, 'first');
if isempty(ndx)
...
SU       = all_units(m);
tf_bin10 = double(neural_data(ndx).time_field);
...
% Dynamic highlight window around time_field
...
% Mirror-pad to avoid edge artifacts when smoothing
...
fill([t fliplr(t)], [mean_fr+sem_fr fliplr(mean_fr-sem_fr)], 'b','EdgeColor','none','FaceAlpha',0.3);
xlim([0 2.5]);
