function plot_single_cell_cue_specific( ...
    nwbAll, all_units, neural_data, ...
...
% ------------------------------------------------------------------
% Enforced behavior:
%   Always collapse all non-preferred images into one group
% ------------------------------------------------------------------
...
encOffset   = 1.0;
maintOffset = 2.5;
totalT      = encOffset + maintOffset;
tVec        = 0:bin_size:(totalT-bin_size);
...
binsMaint   = 0:bin_size:maintOffset;  
n_binsM = numel(binsMaint)-1;
...
if t_end_M <= t_start_M
    t_start_M = (tf_bin-1)*0.1; 
    t_end_M = tf_bin*0.1;
end
...
for g = 1:nGroups
    ...
    for i = 1:numel(idx)
        ...
        if tr <= numel(tsMaint)
            sM = spk(spk>=tsMaint(tr) & spk<tsMaint(tr)+maintOffset) ...
                 - tsMaint(tr) + encOffset;
            if ~isempty(sM)
...
xline(encOffset,'k--','LineWidth',1.5,'HandleVisibility','off');
...
for g = 1:nPSTHgroups
    ...
    fill([tVec fliplr(tVec)], ...
         [mFR35+sFR35 fliplr(mFR35-sFR35)], ...
         col, 'EdgeColor','none','FaceAlpha',0.25,'HandleVisibility','off');

    hLines(g) = plot(tVec, mFR35, 'Color',col,'LineWidth',2, ...
                     'DisplayName',psth_labels{g});
end
...
for k = 1:nT
    ...
    if nTot > 2*pad_bins                 
        ...
        fr   = vsm(pad_bins+1 : pad_bins+nTot);
    else
