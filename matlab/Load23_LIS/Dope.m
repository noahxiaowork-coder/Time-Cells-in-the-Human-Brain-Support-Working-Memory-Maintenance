function Dope(nwbAll, all_units, neural_data_file)
...
for ndx = 1:num_neurons
    ...
    % ----------------- Identify load‑1 preferred / non‑preferred trials
    load1 = find(trial_imageIDs(:,1)~=0 & trial_imageIDs(:,2)==0 & trial_imageIDs(:,3)==0);
    prefTrials    = load1(trial_imageIDs(load1,1)==pImg);
    nonprefTrials = load1(trial_imageIDs(load1,1)~=pImg);
...
function plot_concatenated_single(mat, binSz, totalT, ttl)
    ...
    hold on
    xline(round(1/binSz),'r--','LineWidth',1);   % 1 s divider
    hold off
end
...
if p < 0.05
    ...
    text(1.5, y - 0.5*tick_length, stars, 'HorizontalAlignment', 'center', 'VerticalAlignment', 'bottom', 'FontSize', 14);
end
end
