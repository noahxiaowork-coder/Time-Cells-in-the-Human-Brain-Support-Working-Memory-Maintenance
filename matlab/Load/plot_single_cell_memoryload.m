...
% PLOT_SINGLE_CELL_MEMORYLOAD
% Raster + PSTH split by working memory load (1,2,3 items)
...
sigma_bins = 0.5 / bin_size;                 % same σ you intended (0.5 s)
% padding for mirror extension; can't exceed half the window
pad_bins   = min( ceil(3 * sigma_bins), floor((n_bins-1)/2) );


% ----------------- Determine highlight window -----------------
...
% >>> add this so both raster & PSTH use the same trials <<<
...
% use nClustered instead of nTrials here
...
% --- remove idx*_all; just iterate clustered_idx (already filtered) ---
trialRow = 1;
for i = 1:nClustered
    tr = clustered_idx(i);
    ...
    % If use_correct==true, corr_v(tr) will always be 1; keeps behavior consistent
...
if use_zscore
    ...
    y_label = 'Firing Rate (Hz)'; % updated label
end
...
% --- Plot with handles for legend (fills hidden from legend) ---
h1 = plot_fill(t,m1,s1,[0 0 1]);       % Load 1: blue
h2 = plot_fill(t,m2,s2,[0 0.6 0]);     % Load 2: green
h3 = plot_fill(t,m3,s3,[1 0 0]);       % Load 3: red

% Explicit legend using line handles only
...
% Helper: line + shaded SEM (fill hidden from legend)
...
function sm = smooth_with_padding(vec, k, pad_bins)
    % Mirror-pad a 1D vector in time, convolve, then crop back.
    vec = vec(:)';                        % row
    if numel(vec) <= 2*pad_bins           % guard in case vec is short
        sm = conv(vec, k, 'same');
        return
    end
